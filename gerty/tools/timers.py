...
# In-memory active timers: label -> (Timer, duration_sec)
_active_timers: dict[str, tuple[threading.Timer, int]] = {}
...
class TimersTool(Tool):
    ...
    def execute(self, intent: str, message: str) -> str:
        ...
        if "list" in lower or "show" in lower or "what" in lower and "timer" in lower:
            ...
            lines = [f"• {label}: {dur}s" for label, (_, dur) in _active_timers.items()]
        ...
        if "cancel" in lower or "stop" in lower:
            ...
            for t, _ in _active_timers.values():
                t.cancel()
        ...
        if duration and duration > 0:
            ...
            t.daemon = True
            t.start()
            _active_timers[label] = (t, duration)
