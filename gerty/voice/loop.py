...
# Phrases that end the conversation: go to idle without responding (avoids loop with background noise)
CONVERSATION_END_PHRASES = frozenset({
    "bye", "by", "goodbye", "see ya", "see you", "later", "thanks", "thank you", "thank",
    "cool", "ok", "okay", "stop", "end", "finish", "that's all", "we're done",
    "done", "all done", "that's it", "no more", "nothing", "never mind",
    "stop responding", "thats all", "were done", "enough", "quit", "exit",
})


def _normalize_for_end_phrase(text: str) -> str:
    """Strip and lowercase, remove leading/trailing punctuation (STT often adds . or !)."""
...
def run_voice_loop(
    ...
    *,
    ...
    auto_listen_mode = False  # True when we auto-opened after a response; wake word = go to idle
    ...
    def process_recording():
        nonlocal recording, audio_chunks, recording_started_at, auto_listen_mode
        text = ""
        reply = ""
        try:
            ...
            try:
                ...
                if normalized in CONVERSATION_END_PHRASES or (
                    words and _normalize_for_end_phrase(words[0]) in CONVERSATION_END_PHRASES and len(words) <= 3
                ):
                    logger.info("Voice: conversation end phrase detected: %r", text)
                    if on_user_text:
                        try:
                            on_user_text(text)
                        except Exception as e:
                ...
                if had_successful_response and wake is not None:
                    ...
                    try:
                        ...
                        logger.debug("Capture flush failed: %s", e)
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=1) as ex:
                ...
                try:
                    ...
                    if recording and is_ptt_stop_requested() and not is_processing.is_set():
                        clear_ptt_stop()
                        recording = False
                        is_processing.set()
            ...
            if wake_detected or ptt_start:
                ...
                if is_ptt_stop_requested() and not is_processing.is_set():
                    clear_ptt_stop()
                    recording = False
                    is_processing.set()
                    threading.Thread(target=process_recording, daemon=True).start()
                ...
                # Use longer grace when auto-opened (2s) to catch dominant voice over background
                elapsed = (time.perf_counter() - recording_started_at) if recording_started_at else 0
                grace = VOICE_AUTO_LISTEN_GRACE_SEC if auto_listen_mode else VOICE_WAKE_GRACE_SEC
                if end_detected and elapsed < grace:
                    end_detected = False
                ...
                if (
                    end_detected
                    and len(audio_chunks) >= max(MIN_SPEECH_CHUNKS, MIN_RECORDING_CHUNKS)
